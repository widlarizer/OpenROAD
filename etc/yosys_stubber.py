...
spec = importlib.util.spec_from_file_location(
    "extract_proc", "docs/src/scripts/extract_utils.py"
)
...
err_keys = {"-filter"}
known_keys = (
    untracked_keys | typed_tracked_keys.keys() | untyped_tracked_keys | err_keys
)
decl_proc_name_res = {
    # r'unset_.*',
    r"set_.*_delay",
...
def get_parsers(file):
    ...
    for signature in extracted:
        assert (
            len(signature) == 3 or len(signature) == 4
        ), f"unexpected signature length {len(signature)} of signature {signature}"
        name = signature[0]
        keys = signature[1]
        flags = signature[2]
        checker = False
        ...
        for sig in new_signatures:
            if sig.name == name:
                ...
                if is_doc_decl_defined(name):
                    continue
                assert (
                    sig.positionals == None
                ), f"Already resolved positionals for {name}"
                new_sig = sig
